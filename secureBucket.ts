...
const ALLOWED_REGIONS = ["us-west-2", "ca-central-1"];
...
export class SecureBucket extends pulumi.ComponentResource {
    ...
    constructor(name: string, args: SecureBucketArgs, opts?: pulumi.ComponentResourceOptions) {
        super("datacorp:storage:SecureBucket", name, args, opts);

        const region = aws.getRegionOutput();
        region.name.apply(r => {
            if (!ALLOWED_REGIONS.includes(r)) {
                throw new Error(
                    `Region "${r}" is not allowed. Allowed regions: ${ALLOWED_REGIONS.join(", ")}`,
                );
            }
        });
        ...
        if (args.kmsKeyArn) {
            ...
            kmsKeyId = kmsKeyArn; // When using an external key, expose the ARN as ID
        } else {
            const kmsKey = new aws.kms.Key(`${name}-key`, {
                description: pulumi.interpolate`DataCorp encryption key for customer: ${args.customerName}`,
            ...
            new aws.kms.Alias(`${name}-key-alias`, {
                name: pulumi.interpolate`alias/datacorp-${args.customerName}`,
        ...
        // --- S3 Bucket ---
        const bucketResourceName = region.name.apply(r => `datacorp-${pulumi.output(args.customerName)}-data-${r}`);
